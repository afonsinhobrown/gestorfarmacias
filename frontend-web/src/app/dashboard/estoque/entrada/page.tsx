...
export default function NovaEntradaPage() {
    ...
    const [tipoUnidadeTemp, setTipoUnidadeTemp] = useState<'CAIXA' | 'UNIDADE'>('CAIXA');
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
                            ...
                            <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 shadow-xl rounded-lg mt-1 z-10 max-h-60 overflow-y-auto">
                                {produtosBusca.map((item: any) => (
                                    <button
                                        ...
                                        className="w-full text-left p-3 hover:bg-blue-50 border-b border-gray-50 flex flex-col"
                                    >
                                        <span className="font-bold text-gray-800">{item.produto?.nome || item.nome}</span>
                                        <span className="text-[10px] text-gray-500 uppercase font-medium">
                                            {item.produto?.fabricante || item.fabricante || 'Fabricante N/D'} • {item.produto?.pais_origem || item.pais_origem || 'Origem N/D'}
                                        </span>
                                    </button>
                                ))}
                            </div>
                    ...
                    {prodTemp && (
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-center justify-between">
                            <div>
                                <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Produto Selecionado</p>
                                <h4 className="font-bold text-blue-900">{prodTemp.produto?.nome || prodTemp.nome}</h4>
                                <div className="flex gap-3 mt-1">
                                    <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-bold uppercase tracking-tight">Fabricante: {prodTemp.produto?.fabricante || prodTemp.fabricante || 'N/D'}</span>
                                    <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-bold uppercase tracking-tight">Origem: {prodTemp.produto?.pais_origem || prodTemp.pais_origem || 'N/D'}</span>
                                </div>
                            </div>
                            <button onClick={() => {setProdTemp(null); setBuscaTermo('');}} className="p-2 text-blue-400 hover:text-blue-600 transition-colors">
                                <X size={20} />
                            </button>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div className="space-y-1">
                            <label className="text-xs font-bold text-gray-500 uppercase">Comprar em:</label>
                            <select
                                value={tipoUnidadeTemp}
                                onChange={e => setTipoUnidadeTemp(e.target.value as any)}
                                className="w-full p-2 border border-blue-200 bg-blue-50/20 rounded-lg outline-none font-bold text-xs"
                            >
                                <option value="CAIXA">CAIXAS</option>
                                <option value="UNIDADE">UNIDADES/CARTEIRAS</option>
                            </select>
                        </div>
                            ...
                            {itens.map((item, idx) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                    <td className="py-3 px-4">
                                        ...
                                        <span className="text-[9px] text-blue-600 font-black uppercase">{item.tipo_unidade === 'CAIXA' ? 'Compra em Caixas' : 'Compra Avulsa (Carteiras)'}</span>
                                    </td>
                                    <td className="py-3 px-4 text-center font-bold">
                                        {item.quantidade} {item.tipo_unidade === 'CAIXA' ? 'cx' : 'un'}
                                    </td>
